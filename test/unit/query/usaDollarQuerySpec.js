(function() {
    "use strict";
    describe('std-usa-dollar-query', function() {
        var compile,
            doc,
            scope,
            timeout,
        ...
        var compileAndAppendElement = function() {
            ...
            var elm = angular.element('<std-usa-dollar-query label="Foo Bar" field="searchFields.Field1"></std-usa-dollar-query>');
            elm.appendTo(searchGroup);
            elm = compile(elm)(scope);
            scope.$digest();

            isoScope = elm.isolateScope();
            ctrl = elm.controller('stdUsaDollarQuery');
        };

        beforeEach(module('query.builder'));
        beforeEach(module('query.data.model'));
        beforeEach(module('tru.search.group'));
        beforeEach(module('std.usa.dollar'));
        beforeEach(module('std.operator.lookup'));
        beforeEach(module('std.usa.dollar.query'));

        beforeEach(inject(function ($rootScope, $templateCache, $compile, $timeout, $document, queryDataModel) {
            $templateCache.put('src/templates/query/std-usa-dollar-query.html', __html__['src/templates/query/std-usa-dollar-query.html']);
