(function() {
    "use strict";
    describe('std-or-list-checkbox-query', function() {
        var compile,
            doc,
            scope,
            timeout,
        ...
        var compileAndAppendElement = function() {
            ...
            var elm = angular.element('<std-or-list-checkbox-query label="Foo Bar" field="searchFields.Field1"></std-or-list-checkbox-query>');
            elm.appendTo(searchGroup);
            elm = compile(elm)(scope);
            scope.$digest();

            isoScope = elm.isolateScope();
            ctrl = elm.controller('stdOrListCheckboxQuery');
        };

        beforeEach(module('query.data.model'));
        beforeEach(module('tru.search.group'));
        beforeEach(module('std.operator.lookup'));
        beforeEach(module('std.or.list.checkbox.query'));

        beforeEach(inject(function ($rootScope, $templateCache, $compile, queryDataModel) {
            $templateCache.put('src/templates/query/std-or-list-checkbox-query.html', __html__['src/templates/query/std-or-list-checkbox-query.html']);
        ...
        describe('operator - click', function() {
            ...
            it('clicking icon should select all checkboxes when property.value is undefined and no checkboxes are checked', function () {
                compileAndAppendElement();
                var inputs = document.getElementsByTagName('input');
                var icon = document.getElementsByTagName('i')[0];
                icon.click();
                for (var i = 0; i < inputs.length; i++) {
                    expect(inputs[i].checked).toBe(true);
                }
            });
        });

        describe('operator - appearance', function() {
            it('should have class ttl-operator-icon-has-value when one or more checkboxes are checked', function () {
                dataModel.defaultValue = [1,2];
                compileAndAppendElement();
            ...
            it('should not have class ttl-search-icon-has-value when no checkboxes are checked', function () {
                compileAndAppendElement();
                var div = document.getElementsByClassName('ttl-operator-icon-wrapper')[0];
        ...
        describe('nullable', function() {
            it('when isNullable has value, the checkbox list should append an option with a label of Null', function () {
